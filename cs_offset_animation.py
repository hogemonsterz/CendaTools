...
class ProcessTimer:
    ...
    def finish(self):
        print('%s: %f' % (self.name, time.time() - self.start))


class OffsetAnimationPanel(bpy.types.Panel):
    bl_label = "Offset"
    bl_idname = "ANIM_PT_OFFSET_ANIM_PANEL"
    ...
    def draw(self, context):
        ...
        # Second button
        row = layout.row(align=True)
        row.operator("anim.offset_animation_set", icon="ARMATURE_DATA", text="Offset").reference = False
        ...
        if bpy.context.active_object.type == 'ARMATURE':
            ...
            for bone in armature.bones:
                ...
                currentRotation = Vector(
                    (bonePose.rotation_euler[0], bonePose.rotation_euler[1], bonePose.rotation_euler[2]))
                currentQRotation = Vector((bonePose.rotation_quaternion[0], bonePose.rotation_quaternion[1],
                                           bonePose.rotation_quaternion[2], bonePose.rotation_quaternion[3]))
                currentScale = Vector((bonePose.scale[0], bonePose.scale[1], bonePose.scale[2]))

                if (self.reference):
                    save_offset(currentLocation, currentRotation, currentScale, currentQRotation, bonePose)

                else:
                    apply_offset(currentLocation, currentRotation, currentScale, currentQRotation, bonePose)
...
def save_offset(currentLocation, currentRotation, currentScale, currentQRotation, bonePose):
    # BONES #
    if bpy.context.active_object.type == 'ARMATURE':
...
def apply_offset(currentLocation, currentRotation, currentScale, currentQRotation, bonePose):
    # location
    ...
    # do offset
    offset_animation(locDifference, rotDifference, qRotDifference, sclDifference, bonePose)
...
## ONLY FOR SET OFFSET ##############################################################
def offset_animation(locDifference, rotDifference, qRotDifference, sclDifference, bonePose):
    ...
    for j in range(numKeyframes):
        ...
        if (scn.UseRange):
            if ((key.co.x >= scn.StartRange) and (key.co.x <= scn.EndRange)):
                key.co.y -= offsets[array_index]
                key.handle_left.y -= offsets[array_index]
